...
export const FileUploaderModal: React.FC<FileUploaderModalProps> = ({ isOpen, onClose, onUploadComplete, initialSubject }) => {
    ...
    const subjects: Subject[] = ['Accounting', 'ICT', 'Business Studies', 'Economics', 'Law'];
    ...
    const onDrop = useCallback(async (acceptedFiles: File[]) => {
        ...
        if (successCount > 0) {
            ...
            setTimeout(() => {
                ...
                setSuccess(false);
                setUploadStatus('');
                // Don't reset subject if it was passed as initialSubject
                if (!initialSubject) setSelectedSubject(null);
            }, 2000);
        } else if (failCount > 0) {
            setError(specificError || 'Failed to upload files. Please try again.');
        }

    }, [onClose, onUploadComplete, selectedSubject, initialSubject]);
                                    ...
                                    <div className="grid grid-cols-3 gap-2">
                                        {subjects.map((subj) => (
                                            <button
                                                ...
                                                disabled={isUploading || !!initialSubject}
